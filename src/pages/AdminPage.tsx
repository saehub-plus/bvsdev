...
import { 
  Project, ProjectPage, getProjects, addProject, 
  updateProject, deleteProject, uploadImage 
} from '../lib/firebase';
import { 
  PlusCircle, LogOut, Trash2, Edit, Save, X, 
  Image, Loader2, FileText, Calendar, Link as LinkIcon, Code
} from 'lucide-react';
      ...
      const pagesWithImages = await Promise.all(
        formData.pages.map(async (page) => {
          if (page.imageFile) {
            ...
            return { ...page, imageUrl: pageImageUrl, imageFile: undefined };
          }
          return page;
        })
      );
      ...
      <div className="min-h-screen py-16 bg-cyber-black">
        <div className="container mx-auto px-4">
          <div className="flex justify-between items-center mb-10">
            <h1 className="text-3xl font-display text-neon-green">Painel Administrativo</h1>
            <button 
              onClick={logout}
          ...
          <div className="tech-container mb-8">
            <div className="terminal-header border-b border-neon-green/30 mb-6 pb-2 flex items-center justify-between">
              <div className="flex items-center">
                ...
                <div className="font-mono text-neon-green/70 text-sm">admin_dashboard.js</div>
              </div>
            ...
            {loading ? (
              ...
              <div className="animate-fade-in">
                <h2 className="text-xl font-display text-neon-green mb-4">Adicionar Novo Projeto</h2>
                <form onSubmit={handleAddProject}>
                  <div className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
                        <label htmlFor="title" className="block text-foreground/80 mb-2 font-mono">Título do Projeto</label>
                        <input
                          id="title"
                          name="title"
                      ...
                      <div>
                        <label htmlFor="date" className="block text-foreground/80 mb-2 font-mono">Data de Criação</label>
                        <input
                          type="date"
                          id="date"
                          name="date"
                    ...
                    <div>
                      <label htmlFor="description" className="block text-foreground/80 mb-2 font-mono">Descrição</label>
                      <textarea
                        id="description"
                        name="description"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
                        <label htmlFor="link" className="block text-foreground/80 mb-2 font-mono">Link do Projeto</label>
                        <input
                          id="link"
                          name="link"
                          value={formData.link}
                      ...
                      <div>
                        <label htmlFor="projectImage" className="block text-foreground/80 mb-2 font-mono">Imagem do Projeto</label>
                        <input
                          type="file"
                          id="projectImage"
                          ref={projectImageRef}
                    ...
                    <div>
                      <label className="block text-foreground/80 mb-2 font-mono">Tecnologias</label>
                      <div className="flex items-center">
                        <input
                          id="techInput"
                          value={techInput}
                      ...
                      <div className="flex flex-wrap gap-2 mt-3">
                        {formData.technologies.map((tech, index) => (
                          ...
                          >
                            <span className="text-neon-green text-sm">{tech}</span>
                            <button
                              type="button"
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-3">
                        <label className="block text-foreground/80 font-mono">Páginas do Projeto</label>
                        <button
                          type="button"
                          onClick={() => {
                      ...
                      {formData.pages.length > 0 ? (
                        <div className="space-y-3 mb-4">
                          {formData.pages.map((page, index) => (
                            ...
                            >
                              <div>
                                <h4 className="text-neon-green font-mono">{page.name}</h4>
                                <p className="text-foreground/70 text-sm truncate max-w-md">{page.features}</p>
                              </div>
                        ...
                        <div className="text-center py-6 border border-dashed border-neon-green/20 rounded mb-4">
                          ...
                          <p className="text-foreground/50">Nenhuma página adicionada</p>
                        </div>
                      )}
                      
                      {isAddingPage && (
                        <div className="border border-neon-green/30 rounded p-4 mb-4 animate-fade-in">
                          <h4 className="text-neon-green font-mono mb-3">Adicionar Página</h4>
                          <div className="space-y-4">
                            <div>
                              <label htmlFor="pageName" className="block text-foreground/80 mb-1 font-mono text-sm">Nome da Página</label>
                              <input
                                id="pageName"
                                name="name"
                            ...
                            <div>
                              <label htmlFor="pageFeatures" className="block text-foreground/80 mb-1 font-mono text-sm">Funcionalidades</label>
                              <textarea
                                id="pageFeatures"
                                name="features"
                            ...
                            <div>
                              <label htmlFor="pageImage" className="block text-foreground/80 mb-1 font-mono text-sm">Imagem da Página</label>
                              <input
                                type="file"
                                id="pageImage"
                                ref={pageImageRef}
                      ...
                      {isEditingPage && (
                        <div className="border border-neon-green/30 rounded p-4 mb-4 animate-fade-in">
                          <h4 className="text-neon-green font-mono mb-3">Editar Página</h4>
                          <div className="space-y-4">
                            <div>
                              <label htmlFor="pageName" className="block text-foreground/80 mb-1 font-mono text-sm">Nome da Página</label>
                              <input
                ...
                {projects.length > 0 ? (
                  <div className="space-y-6">
                    {projects.map((project) => (
                      <div key={project.id} className="border border-neon-green/20 rounded p-4 hover:border-neon-green/40 transition-colors">
                        <div className="flex flex-col md:flex-row gap-4">
                          ...
                          <div className={project.imageUrl ? "md:w-3/4" : "w-full"}>
                            <h3 className="text-neon-green font-display text-xl mb-2">{project.title}</h3>
                            ...
                            <p className="text-foreground/70 text-sm line-clamp-2 mb-3">{project.description}</p>
                  ...
                  <div className="text-center py-10">
                    ...
                    <p className="text-foreground/70 mb-4">Nenhum projeto cadastrado</p>
                    <button
